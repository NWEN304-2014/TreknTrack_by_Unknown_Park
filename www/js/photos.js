		...
		if(lastmoddate!=null && lastmoddate!='' && lastmoddate!='Invaid Date'){
			...
			for(var p in photos){
				if(photos.hasOwnProperty(p)){
					...
					var diff = date-lasmoddate;
					if(date == lastmoddate || (diff <= 1000 && diff >= -1000)){
						// alert('get!!! '+p);
						pURI = p;
...
function deletePhoto(){
	...
	if(photoURI != null && photoURI != ''){
		...
		$.when(photos,sharedPhotos,pURI).done(function (photos,sharedPhotos,pURI){
			...
			// ***when testing with Sony Xperia J:
...
function uploadPhoto(){
	if(photoURI!='' && photoURI!=null){
		...
		var pos = photos[photoURI].coords;
		if(pos == ''){
		...
		//convert photo to base64 string, then upload using FileTransfer
		//create hidden html <img> element and <canvas> element to get image source
		//and convert it to base64 string in canvase element
